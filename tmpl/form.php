...
<form name="form" id="my_form" method="POST" action="<?php echo add_query_arg( 'action', 'confirm' ); ?>">
	...
	<table>
		<?php 	foreach($event_cfg['tickets'] as $i => $t): 
			  if($i >= $event_cfg['tickets'][$i]['amount'] ) 
				$u = true;
			  else
			  	$u = false;
	...
	<table id="table_participants" style="width:100%">
		...
		?>
				<tr name="entry">
					...
					<td valign="top"><input type="text" name="users[<?php echo $id ?>][email]"  class="required" value="<?php echo esc_attr($user['email']) ?>"/></td> <!--class="required_email-->
					<td valign="top"><input type="text" name="users[<?php echo $id ?>][phone]"  class="required" value="<?php echo esc_attr($user['phone']) ?>"/></td>
					<?php if($id > 1): ?>
		...
		<?php else: ?>
			<tr name="entry">
				...
				<td valign="top" style="width:32%"><input type="text" name="users[1][email]" class="required"/></td> <!--class="required_email"-->
				<td valign="top" style="width:32%"><input type="text" name="users[1][phone]" class="required" /></td>
				<td style="width:4%">&nbsp;</td>
			</tr>
		<?php endif ?>
		</table>
		...
		<div id="form_company" <?php echo !empty($_SESSION['FORM']['COMPANY']) ? 'style="display:block"' : '' ?>>
			...
			<table cellpadding="3">
				...
				<tr>
					...
					<td><input type="text" name="company[person_email]" class="required" size="30" value="<?php echo esc_attr(@$_SESSION['FORM']['COMPANY']['person_email']) ?>"  /></td>		
				</tr><!--class="required_email"*-->
				<tr>
			</table>
		<br />
		</div>
	...
	jQuery(document).ready(function($) {
		...
		$("#button_add").click(function() {
			...
			$('<tr>'
				...
				+ '<td valign="top"><input type="text" name="users['+id+'][email]" class="required"/></td>' /*class="required_email"*/

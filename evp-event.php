...
function wpe_form() {
	...
	$ticket_types = $wpdb->get_results("SELECT count(*) as count, ticket_type FROM {$wpdb->prefix}event_invoices GROUP by ticket_type", ARRAY_A);	
	$used = array();
	
	foreach($ticket_types as $t) {
		$used[$t['ticket_type']] = $t['count'];
	}
...
function wpe_confirm() {
	...
	if(!empty($_POST) ) {
		...
		$used_count = $wpdb->get_var("SELECT count(*) as c FROM {$wpdb->prefix}event_invoices WHERE ticket_type = '$tid'");
...
function wpe_payment_redirect() {
	...
	if(isset($_SESSION['FORM']['USERS']) && !empty($_SESSION['FORM']['USERS']) && empty($_SESSION['FORM']['ERROR'])) {
        ...
        if($data['payment_type'] == 'prebill') {
            ...
            $pdf_url =  WP_PLUGIN_URL . '/evp-event/pdf/' . $invoice_id .'.pdf';
...
function wpe_payment_callback($web2pay = true, $invoice_id = null) {
	...
	$invoice =  $wpdb->get_row("SELECT id, group_id, amount 
				    ...
				    WHERE id = {$invoice_id} AND paid_in = 0  
				    LIMIT 1");
